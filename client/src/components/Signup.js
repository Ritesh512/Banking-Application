...
function Signup(){
    ...
    async function submit(){
        if(!name || !email || !password || !confirmPass || !mob|| !pan || !addcard){
            setErr(true);
        ...
        let result =await fetch("http://localhost:8000/register",{
            method:"post",
            body:JSON.stringify({name,email,password,addcard,pan,mob,dob,proilePass}),
            ...
            <section className="gridSignUp">
                ...
                <div>
                    <div className='flexSignup'>
                        ...
                        <input className='inputBox' type="text" name="adh" placeholder="Aadhar No" 
                        value={addcard} 
                        onChange={AddCard}
                        />
                    </div>
                    {adderr && <span className="error">Enter valid Addahar No.</span>}
                    {error  && !addcard && <span className="error">Enter valid Addahar No.</span>}
                </div>
                <div>
                    <div className='flexSignup'>
                        <label>Email: </label>
                ...
                <div>
                    <div className='flexSignup'>
                        <label>Pan No.: </label>
                        <input className='inputBox' type="text" name="pan" placeholder="Pan No"
                        value={pan}
                        onChange={Pan} />
                    </div>
                ...
                <div>
                    <div className='flexSignup'>
                        ...
                        <input className='inputBox' type="text" name="number" placeholder="phone" 
                        value={mob}
                        onChange={Mob}
                        />
                    </div>
                    {moberr && <span className="error">Enter valid Mobile No.</span>}
                    {error && !mob && <span className="error">Enter valid Mobile No.</span>}
                </div>

                <div>
                    <div className='flexSignup'>  
                        <label>DOB: </label>
                        <input className='inputBox' type="date" name="number" placeholder="phone" 
                        value={dob}
                        onChange={Dob}
                        />
                    </div>
                    {error && !dob && <span className="error">Enter valid DOB</span>}
                </div>
